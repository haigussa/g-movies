...
const MovieDetail = (props) => {
  ...
  const movieGenres = movieDetail.genres.length
    ? movieDetail.genres.map((genre) => <li key={genre.name}>{genre.name}</li>)
    : "";
  ...
  const movieDirectors = crews.length
    ? crews.filter((crew) => {
    ...
    return (
      <>
        ...
        >
          <MovieInfoRow
            rating={
              movieDetail.movie.vote_average > 0
                ? movieDetail.movie.vote_average
                : "N/A"
            }
          ...
          <div className="movieDescriptionWrapper">
            <div className="title">
              ...
              <span className="tagline">{movieDetail.movie.tagline}</span>
            </div>
        ...
        <StyledCastGrid>{movieCredits}</StyledCastGrid>

        <button
          onClick={scrollToTop}
          style={scrolled ? showTopButton : hideTopButton}
        >
          <FaRegArrowAltCircleUp size="3rem" />
        </button>
      </>
    );
  } else if (loading) {
